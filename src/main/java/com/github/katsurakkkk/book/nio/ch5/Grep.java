package com.github.katsurakkkk.book.nio.ch5;
...
/**
 * 191 A file searching class, similar to grep, which returns information about lines matched in the specified files. Instances of this
 * class * are tied to a specific regular expression pattern and may be applied * repeatedly to multiple files. Instances of Grep are thread
 * safe, they may be shared.
 *
...
public class Grep {
	 ...
	 * 
	 * @param pattern
	 *            A java.util.regex.Pattern object specifying the pattern to search for.
	 */
	 ...
	 * 
	 * @param regex
	 *            The regular expression string to compile into a Pattern for internal use.
	 * @param ignoreCase
	 *            If true, pass Pattern.CASE_INSENSITIVE to the Pattern constuctor so that seaches will be done without regard to alphabetic
	 *            case. Note, this only applies to the ASCII character set. Use embedded expressions to set other options.
	 */
	 ...
	 * 
	 * @param file
	 *            A File object denoting the file to scan for the regex given when this Grep instance was constructed. 192
	 ...
	 * 
	 * @param file
	 *            A String filename denoting the file to scan for the regex given when this Grep instance was constructed.
	 * @return A type-safe array of Grep.MatchedLine objects describing the lines of the file matched by the pattern.
	 * @exception IOException
	 *                If there is a problem reading the file.
	 */
	...
	/**
	 * Test code to run grep operations. Accepts two command-line options: -i or --ignore-case, compile the given pattern so that case of
	 * alpha characters is ignored. Or -1, which runs the grep operation on each individual file, rather that passing them all to one
	 * invocation. This is just to test the different methods. The printed ouptut is slightly different when -1 is specified.
	 */
